...
气泡保留周额度、余额和重置时间，新增绿色的「今日已用 / 剩余」进度条，不显示单独的日均建议行。后台参考量为 **周剩余额度 ÷ 距周重置的剩余天数**；不足一天按一天计算。今日已记录的周用量增量除以参考量得到进度，随剩余额度和时间动态变化。超过 100% 时显示实际比例，进度条填满并变为提醒色。它是使用节奏参考，不是官方额外发放的日额度。

例如：周剩 60%、剩余 5 天，后台每日参考量为周总量的 12%；今天记录到使用周总量的 3%，显示已用 25%、剩余 75%。

按本机日期累计，记录保存在 `%LOCALAPPDATA%\MomoCodexPet\daily-usage.json`，重启后继续。接口没有日用量历史，首次启用或跨日停机后只能从新读数开始记录：气泡直接显示已记录时段的具体百分比，不加不等号，最多保留两位小数；记录范围和起始时间仅在悬停说明中展示。周累计读数未增加时，已记录的今日用量保持不变。午夜附近的同步差额存在采样误差；周窗口、套餐变化或周用量回退时重新建立基线。缺失或过期数据显示 —；离线时保留上次进度。
