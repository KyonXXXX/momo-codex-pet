...
按本机日期累计，记录保存在 `%LOCALAPPDATA%\MomoCodexPet\daily-usage.json`，重启后继续。接口没有日用量历史，首次启用或跨日停机后只能从新读数开始记录：此时用 ≥ / ≤ 标明已记录部分的上下限，悬停可查看起始时间。午夜附近的同步差额存在采样误差；周窗口、套餐变化或周用量回退时重新建立基线。缺失或过期数据显示 —；离线时保留上次进度。
