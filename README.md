...
上图使用合成演示数据，不含真实账号的额度信息。默认界面没有名称栏，主文字 10–13 px，辅助文字 9 px。气泡居中放在角色头顶，默认开启，重启后记住选择。完整窗口为 256 × 410（逻辑像素），角色保持 256 × 256 的绘制区域；小巧模式为 220 × 346，气泡仍保持可读尺寸。关闭气泡会收起上方空间并保持角色位置，屏幕边缘会自动调整。拖动范围按当前动画的不透明像素及可见控件计算，透明边距可伸出屏幕，左右贴边只留约 2 px 余量；动画切换时会检查边界，避免新姿势被截断。
